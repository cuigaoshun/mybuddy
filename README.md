...
当前主链路仍以飞书为主：通过飞书 `websocket` 接收消息，通过飞书开放平台 API 发送消息，在中间接入 Agent 编排、记忆检索和 PostgreSQL 存储抽象，形成一个最小可运行闭环。

同时，仓库里已经开始落微信接入的最小运行态能力，包括扫码登录 API、`wechat_account` 运行态存储、启动后按活跃账号拉起长轮询 runner，以及通过统一消息模型接入现有路由主链路。它还处于增量接入阶段，不应被理解成已经完成的平台泛化。
...
- 在不破坏飞书主链路的前提下，增量接入微信扫码登录、长轮询收消息和统一发送路径。
...
当前新增的微信接入能力与这些目录职责的关系是：

- 微信长轮询 runner 仍然通过应用启动阶段托管生命周期，但运行态更新不直接塞进 `bootstrap`。
- 微信原始消息仍然会先走 `gateway/dispatch` 做归一化，再进入现有 `router` 主链路。
- 微信发送上下文与账号运行态会通过独立存储抽象维护，不直接混进 `chat_session_info`。
...
微信接入的增量方案与当前已拍板实现口径，请看：

- `docs/wechat-migration-plan/README.md`
